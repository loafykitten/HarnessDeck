...
**`server/harness/registry.ts`** — id, label, binary, skills dir, instructions
...
`web/src/stores/state.svelte.ts`).
...
Add an entry to `HARNESSES` in `server/harness/registry.ts`:
...
`web/src/types/api.ts` (`HarnessId` type) plus the `DEFAULT_HARNESSES`
fallback in `web/src/stores/state.svelte.ts`.
...
- Add a `server/usage/foo.ts` modeled on `server/usage/codex-usage.ts`: produce a
  `MonthUsage` (the shape `server/usage/claude.ts` exports — daily tokens/cost,
...
- Surface it in the `/api/usage` handler in `server/usage/routes.ts` (add a field
  next to `codex:`), keep it `Promise.allSettled`-tolerant, and give it a
  60s cache + serve-stale-on-error like the others.
- Add a card to `web/src/features/dashboard/` (the Codex card is
...
(`server/usage/codex-auth.ts: getCodexMode/setCodexMode`, `PUT /api/codex/mode`). If a
...
- **Updater**: add an entry to `updaters` in `server/harness/updates.ts` — the
  ...
  update. The frontend chip (`web/src/features/dashboard/UpdateChip.svelte`,
...
- **Plan**: bespoke per vendor, like usage. Claude's comes from the
  Anthropic OAuth profile (`server/usage/claude.ts`); Codex's is decoded from the
  `chatgpt_plan_type` / `chatgpt_subscription_active_until` claims in the
  id_token JWT in `~/.codex/auth.json` (`getCodexPlan` in
  `server/usage/codex-auth.ts`). Expose `{label, renewsAt}` in the harness's `/api/usage`
  slice and render it with the `PlanBadge` component; omit it (null) when the
...
Harness badge colors live in `web/src/app/styles/chips.css` (`.hx.claude`, `.hx.codex`).
...
| Concern | File |
|---|---|
| Registry (single source of truth) | `server/harness/registry.ts` |
| Sessions/tmux + status regex use | `server/terminal/sessions.ts` |
| Skills multi-harness ownership | `server/skills/skills.ts` |
| Settings + instructions IO | `server/config/config.ts` |
| Codex usage + auth toggle + plan | `server/usage/codex-usage.ts` + `codex-auth.ts` |
| Claude usage | `server/usage/claude.ts` |
| Per-harness self-updaters | `server/harness/updates.ts` |
| Routes | `server/index.ts` + per-feature `routes.ts` |
| Frontend types + API client | `web/src/types/api.ts` + `web/src/lib/api.ts` |
| Harness list in app state | `web/src/stores/state.svelte.ts` |
| Dashboard usage cards | `web/src/features/dashboard/` |
| Session harness picker | `web/src/features/project/ProjectView.svelte` |
| Skills filter/sync/badges | `web/src/features/skills/SkillsView.svelte` |
| Config harness cycling | `web/src/features/config/ConfigView.svelte` |
| Badge colors | `web/src/app/styles/chips.css` |
